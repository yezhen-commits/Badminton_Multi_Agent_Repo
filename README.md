...
### Future Improvement
Current Issue:
- Agent took very long to return an answer, especially during MCP call/API call due to the data being big
- Also, the data in the database are generated by ChatGPT, so it is not accurate

Solution Attempt:
- Create a vector database for RAG, create by taking information from sportrader and add it into vector database
